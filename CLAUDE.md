...
| **2026-05-05 23:15:12** | **增量刷新** | **Universal full-text loader (HTML / GitHub README / PDF) + 评分用全文 + chat prompt cap 提到 200K + `[all]` 聚合 extras**。① **`backend/kb/processing/pdf.py` 重命名为 `fulltext.py`**：三 loader 按 source 派发——PDF（保留，pypdf）/ HTML（trafilatura，需 `[fulltext]` extras）/ GitHub project（REST API `Accept: application/vnd.github.raw`，自动剥 `.git` 后缀）。三路均**httpx 流式 + 早期 abort**（HTML 8MB / PDF 20MB / README 4MB cap，超限统一返回 `""` 不污染缓存——三路 oversize 行为对齐）。`Paper.full_text` 缓存上限从 120K → **200K 字符**。新增内部工人 `_ensure_cached(paper_id)`（单 SessionLocal，prefetch 用，避免每行多开 DB 连接）；`fetch_full_text` 公开 API 与 chat 调用契约不变（含 abstract fallback）。② **采集尾步骤**（`kb/ingestion/run.py`）：`run_ingestion()` 末尾自动调 `prefetch_pending_full_text()`（4 worker `ThreadPoolExecutor`，仅 `source_type ∈ {blog, project, talk} AND full_text == ""`，幂等——已缓存的行直接 skip；包 try/except 不阻塞 ingest 总数）。③ **评分用全文**（`kb/processing/llm.py::summarize_and_score`）：改用 `paper.full_text or paper.abstract`，新增 `_SCORING_BODY_CAP = 30_000`（明显改善 blog / project 评分质量——之前他们只能根据 200 字 og:description / GitHub trending blurb 打分）；prompt 字段名 `Abstract:` → `Content:`。④ **chat prompt cap 提到 200K**（`kb/main.py::_SOURCE_TEXT_PROMPT_CAP`）：从 60K → **200K**，与 `_MAX_EXTRACTED_CHARS` 对齐——一篇满血缓存的文章在 chat 端不被裁剪（200K chars ≈ 50K tokens，仍在 Claude / GPT-4o 上下文以内）。⑤ **新依赖管理**：`pyproject.toml` 新增 `[fulltext]` extras（`trafilatura>=1.12.0`）+ **聚合 `[all]` extras**（`gpgpu-kb[ml]` + `gpgpu-kb[llm-cloud]` + `gpgpu-kb[fulltext]` 自引用，pip 与 setuptools 都支持），是给端用户的便利安装路径——`pip install -e '.[all]'` 一行替代三行。⑥ **Docker 默认改 `INSTALL_EXTRAS=all`**：`backend/Dockerfile` ARG 默认值 + `docker-compose.yml` `${BACKEND_INSTALL_EXTRAS:-all}` 兜底 + `.env.docker.example` 默认值 + README 部署章节同步更新；`Dockerfile` 内 sentence-transformers 预下载条件从 `,ml,` 改为 `,(ml\|all),` 兼容新默认。⑦ **新运维脚本 `kb/scripts/backfill_full_text.py`**——一次性回填旧非论文行 `full_text`（`--dry-run` / `--limit` / `--source-type`，4 worker 并发，**不重新评分**避免 LLM 账单激增；用户先跑这个，再视情况手动跑 rescore）。⑧ `rescore_non_papers.py` 新增 `--include-already-scored` 标志（drop `quality_score == 0.0` filter，配合 backfill 后人工触发重评）。⑨ **测试套件 216 → 242 (+26)**：重命名 `test_processing_pdf.py` → `test_processing_fulltext.py` 并扩到 17 例（PDF 路径全保留 + HTML happy/失败/`stream` mock 契约/oversize 流式 abort + GitHub happy/404/non-github URL/`.git` 剥离 + prefetch 选行 + 幂等）；`test_ingestion_run.py` 加一行 `patch.object(fulltext_mod, "prefetch_pending_full_text", return_value=0)` 确保 spy 不触发真实网络。⑩ **AI 协作约定（编码规范）**：从此**新加非 PDF 全文 loader 必须遵循三条不变量**：① 流式下载 + 早期 abort（任何 cap 不能在 `resp.content` 已 buffered 后才检查）；② 超限统一返回 `""`，不写入 cache，不做字节截断；③ 失败永远返回 `""`，让上层 `_abstract_fallback` 兜底。**新加抓取目标**（如 reddit、HN 等）应作为 `_extract_for_paper` 内的新分支，按 `source_type` / URL 模式 dispatch。**LLM 评分 prompt 修改**：动 `Content:` 字段时同步审视 `_SCORING_BODY_CAP` 是否还合适（30K 是经过 token 预算 / context window 取舍的当前甜点）。**chat prompt 修改**：`_SOURCE_TEXT_PROMPT_CAP` 与 `_MAX_EXTRACTED_CHARS` 应保持对齐或前者 ≤ 后者，否则 chat 拿到的内容会被二次裁剪。⑪ **不影响**：DB schema / migration / chat 多轮 / source-anchored chat / SSE 流式聊天 / fast-expert 双角色 / per-source ingest 冷启动 / sitemap blog / `/api/daily/*` / 前端任何代码全部不动。所有 delta 已通过直接读取 `backend/kb/processing/fulltext.py` / `backend/kb/processing/llm.py` / `backend/kb/main.py` / `backend/kb/ingestion/run.py` / `backend/kb/scripts/{backfill_full_text,rescore_non_papers}.py` / `backend/pyproject.toml` / `backend/Dockerfile` / `docker-compose.yml` / `.env.docker.example` / `README.md` / `backend/tests/test_processing_fulltext.py` / `backend/tests/test_ingestion_run.py` 源码核对。 |
