...
> 于 **`2026-05-05 23:15:12`** 增量刷新（**Universal full-text loader（HTML / GitHub README / PDF）+ 评分用全文 + chat prompt cap 提到 200K + `[all]` 聚合 extras**：① **`backend/kb/processing/pdf.py` 重命名为 `fulltext.py`**——三种 loader 按 source 派发：PDF（保留，pypdf）/ HTML（trafilatura）/ GitHub project（REST API `Accept: application/vnd.github.raw`，自动剥 `.git` 后缀）。三路均走 httpx **流式 + 早期 abort**（HTML 8MB / PDF 20MB / README 4MB cap，超限统一返回 `""` 不污染缓存）。`Paper.full_text` 缓存上限从 120K → **200K** 字符。新增内部工人 `_ensure_cached(paper_id) -> bool`（单 SessionLocal，`prefetch` 用），`fetch_full_text` 公开 API 与既有 chat 调用契约不变（含 abstract fallback）；② **采集尾步骤**：`run_ingestion()` 末尾自动调 `kb.processing.fulltext.prefetch_pending_full_text()`（4 worker `ThreadPoolExecutor`，仅 `source_type ∈ {blog, project, talk} AND full_text == ""`，**幂等**——已缓存的行直接 skip；包 try/except 不阻塞 ingest 总数）；③ **评分用全文**：`kb/processing/llm.py::summarize_and_score` 改用 `paper.full_text or paper.abstract`，新增 `_SCORING_BODY_CAP = 30_000`（明显改善 blog / project 评分质量——之前他们只能根据 200 字 og:description / GitHub trending blurb 打分），prompt 字段名 `Abstract:` → `Content:`；④ **chat prompt cap 提到 200K**：`kb/main.py::_SOURCE_TEXT_PROMPT_CAP` 从 60K → **200K**（与 `_MAX_EXTRACTED_CHARS` 对齐，让一篇满血缓存的文章在 chat 端不被裁剪——200K chars ≈ 50K tokens，仍在 Claude / GPT-4o 上下文以内）；⑤ **新依赖 `[fulltext]` extras**（`pyproject.toml`）：`trafilatura>=1.12.0`。**新增聚合 `[all]` extras**：`gpgpu-kb[ml]` + `gpgpu-kb[llm-cloud]` + `gpgpu-kb[fulltext]`，是给端用户的便利；⑥ **Docker 默认改 `INSTALL_EXTRAS=all`**（`Dockerfile` ARG + `docker-compose.yml` 兜底 + `.env.docker.example`），`Dockerfile` 内 sentence-transformers 预下载 `grep -qE ',(ml\|all),'` 兼容新默认；⑦ **新运维脚本 `kb/scripts/backfill_full_text.py`**——一次性回填旧非论文行的 `full_text`（`--dry-run` / `--limit` / `--source-type`，4 worker 并发，**不重新评分**避免 LLM 账单激增）；⑧ `rescore_non_papers.py` 新增 `--include-already-scored` 标志（drop `quality_score == 0.0` filter，配合 backfill 后人工触发重评）；⑨ **测试套件 216 → 242 (+26)**：重命名 `test_processing_pdf.py` → `test_processing_fulltext.py` 并扩到 17 例（PDF 路径保留 + HTML happy/失败/流式 abort + GitHub happy/404/non-github URL/`.git` 剥离 + prefetch 选行 + 幂等），`test_ingestion_run.py` 加一行 `patch.object(fulltext_mod, "prefetch_pending_full_text", return_value=0)` 确保 spy 不触发真实网络。`/api/chat` 多轮 / source-anchored / SSE 流式 / fast-expert 双角色 / per-source ingest 冷启动 / sitemap blog / `/api/daily/*` 全部不动。所有 delta 已通过直接读取源码核对）。
> 于 **`2026-05-06 00:04:43`** 自适应增量刷新（**docs-only re-sync, no code drift**：核对 `backend/kb/processing/fulltext.py` / `backend/kb/scripts/backfill_full_text.py` / `backend/Dockerfile`（`INSTALL_EXTRAS=all` + `grep -qE ',(ml\|all),'` ST 预下载条件）/ `backend/pyproject.toml`（`[fulltext]` + 聚合 `[all]` extras）/ `docker-compose.yml`（`${BACKEND_INSTALL_EXTRAS:-all}` 兜底）/ `.env.docker.example`（`BACKEND_INSTALL_EXTRAS=all` 默认值）/ `backend/kb/ingestion/run.py`（尾步骤 `prefetch_pending_full_text()` 落位 + 幂等 try/except）/ `backend/kb/main.py` 头部 imports 与 `kb/processing/fulltext.py` 公开 API；全部与 2026-05-05 23:15:12 changelog 描述一致，**无新功能 / 无代码漂移**。仅刷新 `CLAUDE.md` 顶部时间戳与 `.claude/index.json::generated_at` + changelog。识别模块仍为 backend + frontend 两个；Mermaid 图、模块导航面包屑（`[← 返回根]`）、表格索引、覆盖率指标维持原状）。
...
**全文加载在 `backend/kb/processing/fulltext.py`**（前身 `pdf.py`）：三条路径按 source 派发——① **PDF**（arxiv 等，`pypdf`，20MB/30s 上限）；② **HTML**（blog，`trafilatura`，需装 `[fulltext]` extras，8MB/15s 上限，stream + 早期 abort）；③ **GitHub project**（REST API `Accept: application/vnd.github.raw`，4MB 上限，自动剥 `.git` 后缀）。三路均**流式 + 超限返回 `""` 不污染缓存**。结果缓存在 `Paper.full_text`（**20 万字符截断**），分两条触发链：① **采集时主动**——`run_ingestion()` 尾步骤 `prefetch_pending_full_text()`（4 worker，仅 `source_type ∈ {blog, project, talk} AND full_text == ""`，幂等）；② **chat 时按需**——`/api/chat[/stream]` 进入 `paper_id` 锚定模式时 lazy 触发 `fetch_full_text(paper_id)`（含 abstract fallback）。
...
graph TD
    ...
    B --> B3["kb/ingestion · ArXiv / RSS / sitemap_blog / GitHub + 自适应 days_back + 尾步骤 prefetch"]
    B --> B4["kb/processing · LLM (4 providers, fast/expert 双角色, call_llm + stream_llm) + Embeddings + fulltext (PDF/HTML/GitHub)"]
    B --> B5["kb/reports.py · 日报 (按 max(quality,relevance) 排序)"]
    B --> B6["kb/scripts/ · 一次性运维脚本 (rescore_non_papers + backfill_full_text)"]
    B --> B7["tests/ · pytest (~242)"]
    B --> B8["Dockerfile (INSTALL_EXTRAS=all 默认) + .dockerignore + run_api.sh (--timeout-keep-alive 75)"]

    C --> C1["src/app · App Router 页面 (/reports 加 Run-Now 按钮)"]
    C --> C2["src/components · shadcn/ui + chat sidebar/source-picker + ThemeSwitcher + LanguageSwitcher + SourceFilter"]
...
| 路径 | 语言 / 框架 | 一句话职责 | 文档 |
| --- | --- | --- | --- |
| `backend/` | Python 3.12 · FastAPI · SQLAlchemy 2 · ChromaDB · pypdf · trafilatura（可选） | 数据采集（含尾步骤 fulltext prefetch）、LLM 摘要 + 双维度评分（用 full_text）、嵌入索引、REST API（含多轮 RAG、source-anchored chat、**SSE 流式聊天**、**手动 daily pipeline 触发 + SSE 进度**）、日报、运维脚本（rescore + backfill_full_text） | [`backend/CLAUDE.md`](./backend/CLAUDE.md) |
...
pip install -e '.[fulltext]'      # （可选）HTML article extraction（trafilatura，~10MB）
pip install -e '.[all]'           # 聚合：等价 [ml] + [llm-cloud] + [fulltext]
...
python -m kb.scripts.backfill_full_text --dry-run        # 回填旧非论文行的 full_text
python -m pytest tests/ -x -q     # 跑测试 (~242 例，含 SSE / daily-stream / fast-expert / fulltext)
```
...
| `BACKEND_INSTALL_EXTRAS` | `all` | 镜像构建参数（聚合：等价 `ml,llm-cloud,fulltext`）；留空可去掉 ML 栈（~2GB），改 `llm-cloud,fulltext` 仅去 ML |
...
- **后端**：`backend/tests/`（pytest + pytest-asyncio + httpx），约 **~242 个用例，<35 秒，无网络**，详见 `backend/tests/README.md`。本轮新增覆盖：
...
| **2026-05-03 23:00:00** | **增量刷新** | **Browse 页 Source 名称过滤**。① **后端**：新端点 `GET /api/sources` 返回 `[{name, type, count}, ...]`（仅统计 `is_processed=1`，按 count desc）；`schemas.py` 新增 `SourceItem` / `SourcesOut`。`GET /api/papers` 增加 `source_name: str | None`（逗号分隔多值，cap 50 项 + max_length=500），`Paper.source_name.in_([...])` 过滤；与 `source_type` AND 组合；空字符串透明忽略。`/api/papers/search` 不动，保持搜索路径忽略 type/source 过滤的现状。② **前端**：`src/components/source-filter.tsx` 新组件——按 `source_type` 分 4 组（paper/blog/project/talk），每组可折叠，tag 渲染 `name · count` Badge；`typeFilter` 非空时只渲染对应组。Browse 页 (`src/app/page.tsx`) 把 `typeFilter` 与 `selectedSources` 从组件 state 迁到 URL（`?type=...&source=arxiv,openai`），切 type 时 silently drop 不匹配的 source（决策 5 自然推论）。`src/lib/types.ts` 新增 `Source` / `SourcesResponse`；`src/lib/api.ts::listPapers` 接受 `source_name?: string[]`；新增 `listSources()`。i18n 加 3 条 key（`browse.filter.sources` / `.expand` / `.collapse`）。query 模式（`?q=`）下 SourceFilter 不渲染。③ **关键 workaround**：Next 16.2.x 已知 router cache 回归（vercel/next.js#92187）使 `router.push/replace` 在"同 pathname、不同 search"导航时**静默 no-op**（侧栏 Browse 链接被 prefetch 触发缓存命中即复现）。绕过办法：filter state 退回 React `useState`（初始值取自 `useSearchParams()`），URL 同步走 `window.history.replaceState`——SSR/CSR 链接仍可分享，但运行中切 filter 不再依赖被破坏的 router 缓存。④ **测试**：`backend/tests/test_api_smoke.py` +7 例（`/api/sources` 3 例 + `/api/papers?source_name=` 4 例，新增 `_seed_namespaced_source_papers` helper 解决 session-scoped DB 数据累积，断言用唯一 prefix 后缀的 source_name）；`frontend/tests/e2e/browse.spec.ts` +2 例（点 tag → URL 与列表更新；切 type=paper → blog source 被静默 drop）。⑤ **gitignore**：`frontend/.gitignore` 新增 `/test-results/` `/playwright-report/` `/playwright/.cache/`，避免 Playwright 产物污染 git。⑥ **不影响**：DB schema / migration / chat / reports / stats / ingestion / processing 全部不动；既有 paper-card / paper-detail / search / 移动端响应式不变。 |
| **2026-05-03 22:34:43** | **增量刷新** | **Themed i18n frontend shell + 手动触发 daily pipeline 端点（SSE 进度）**。详细 delta 列表见上一版本（i18n / theme 模块、Cream Linen / Walnut Hearth oklch 双主题、`/api/daily/stream` SSE 端点、`_DailyRunState` 单例、`_QueueLogHandler` + `_QueueStdoutWriter`、`_STAGE_PATTERN` 中英 banner、reports 页 Run-Now + 4-stage StagePill、`MAX_LOG_LINES=2000`、6 例 daily-pipeline SSE 测试） |
| **2026-05-05 23:15:12** | **增量刷新** | **Universal full-text loader (HTML / GitHub README / PDF) + 评分用全文 + chat prompt cap 提到 200K + `[all]` 聚合 extras**。详细 delta 列表见上一版本（`pdf.py` → `fulltext.py` 三路 dispatcher / 200K 缓存 cap / `_ensure_cached` 单 SessionLocal worker / `prefetch_pending_full_text()` ingestion 尾步骤 / `summarize_and_score` 用全文 30K cap / chat `_SOURCE_TEXT_PROMPT_CAP=200K` / `[fulltext]` + `[all]` extras / Docker `INSTALL_EXTRAS=all` 默认 / `backfill_full_text.py` 新脚本 / `rescore_non_papers.py` 加 `--include-already-scored` / 测试 216 → 242） |
| **2026-05-06 00:04:43** | **自适应增量刷新（docs-only re-sync）** | **代码与文档已对齐，仅刷新索引**。本次扫描核对：`backend/kb/processing/fulltext.py`（PDF / HTML / GitHub 三路 + `_ensure_cached` + `prefetch_pending_full_text` + 200K cap + 8/20/4 MB stream caps + `_PREFETCH_TYPES = (BLOG, PROJECT, TALK)` + 4-worker 并发，全部存在）/ `backend/kb/scripts/backfill_full_text.py`（`--dry-run` / `--limit` / `--source-type` + `_ensure_cached` 复用 + `_BACKFILL_WORKERS=4` + `is_processed.in_([1,2])` 过滤，存在）/ `backend/Dockerfile`（`ARG INSTALL_EXTRAS=all` + `grep -qE ',(ml\|all),'` ST 预下载条件 + `--timeout-keep-alive 75`，存在）/ `backend/pyproject.toml`（`[fulltext]` 含 `trafilatura>=1.12.0` + 聚合 `[all]` 自引用 `gpgpu-kb[ml]` + `gpgpu-kb[llm-cloud]` + `gpgpu-kb[fulltext]`，存在）/ `docker-compose.yml`（`${BACKEND_INSTALL_EXTRAS:-all}` 兜底，存在）/ `.env.docker.example`（`BACKEND_INSTALL_EXTRAS=all` 默认值，存在）/ `backend/kb/ingestion/run.py`（末尾 `prefetch_pending_full_text()` try/except + `results["fulltext_prefetched"]`，存在）/ `backend/kb/main.py` 头部 imports（`asyncio` / `contextlib` / `hmac` / `re` / `threading` / `deque` 全部就位）。**全部源码与 2026-05-05 23:15:12 changelog 描述一一吻合，无新功能、无代码漂移、无新模块根**。仅在三份 `CLAUDE.md` 顶部追加本次时间戳标记，并把 `.claude/index.json::generated_at` 推到 `2026-05-06T00:04:43Z`、changelog 追加本条 docs-only re-sync 记录。Mermaid 图、模块导航面包屑、表格索引、覆盖率指标维持上一版本不变（仅微调 Mermaid 中 backend 节点的描述，把 `pdf.py` 改写成 `fulltext (PDF/HTML/GitHub)` + `kb/scripts/` 描述追加 `backfill_full_text` + `Dockerfile` 描述追加 `INSTALL_EXTRAS=all 默认`，让结构图也更新到本轮）。 |
